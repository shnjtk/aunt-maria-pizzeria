...
describe('Create order handler', () => {
    ...
    afterEach(() => {
        fakeHttpRequest.uninstall('https');
    });
    ...
    it('should throw an error if request is not valid', () => {
        ...
        expect(() =>
            underTest({ body: { address: '221b Baker Street' } })
        ).toThrow();
    });
        ...
        https.request.pipe(callOptions => {
            ...
            expect(callOptions).toEqual(
                jasmine.objectContaining({
                    protocol: 'https:',
                    slashes: true,
                    host: 'fake-delivery-api.effortlessserverless.com',
                    path: '/delivery',
                    method: 'POST',
                    headers: {
    ...
    it('should not call the DynamoDB DocumentClient.put if Some Like It Hot delivery API request was not successful', done => {
        ...
        https.request.pipe(() =>
            https.request.calls[0].respond(500, 'Server Error', '{}')
        );
    });
    ...
    it('should reject the promise if womething went wrong', done => {
        underTest({
            context: { authorizer: { claims: '' } },
            body: {
                pizzaId: 1,
